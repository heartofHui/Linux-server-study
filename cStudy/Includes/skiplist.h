...
public:
	...
	void getRangeByScore(const int64_t& start,const int64_t& end, std::vector<NodeValue>& resList, const bool& withscores=false, const int64_t& limitCount=-1);
	...
	void getRangeByRank(const int64_t& start,const int64_t& end, std::vector<NodeValue>& resList, const bool& withscores=false, const int64_t& limitCount=-1,const bool reverse=false);
...
void SkipList::deleteRangeByRank(const int64_t& start,const int64_t& end, std::vector<int64_t>& resList, const int64_t& limitCount)
{
	...
	{
		while(current->level[i].forward->score!=INT64_MAX && (through+current->level[i].span) < tmpStart)
		{
	...
	while(current && current->score != INT64_MAX && (index <= numPre) && (limitCount==-1 || (limitCount != -1 && index < limitCount)))
	{
...
int64_t SkipList::getRank(const int64_t& member,const int64_t& score)
{
	...
	for (int i = listLevel - 1; i >= 0; i--)
	{
		while(current!=NULL && current->level[i].forward->score!=INT64_MAX &&
...
void SkipList::deleteRangeByScore(const int64_t& start,const int64_t& end, std::vector<int64_t>& resList, const int64_t& limitCount)
{
	...
	for (; i >= 0; i--)
	{
		while (current->level[i].forward->score != INT64_MAX
...
SKNode* SkipList::getNodeByRank(const int64_t& rank)
{
	...
	for (int i = listLevel - 1; i >= 0; i--)
	{
		while(current->level[i].forward->score!=INT64_MAX && (through+current->level[i].span)<=rank)
		{
			through += current->level[i].span;
			current = current->level[i].forward;
		}
...
void SkipList::getRangeByRank(const int64_t& start,const int64_t& end, std::vector<NodeValue>& resList, const bool& withscores, const int64_t& limitCount, const bool reverse)
{
	int64_t tmpStart = (start < 0) ? (length + start-2) : start;
	int64_t tmpEnd = (end < 0) ? (length + end-2) : end;
	...
	int64_t limitTmp = limitCount;
	//int64_t index = 0;
	int64_t numPre = reverse ? (tmpStart - tmpEnd + 1) : (tmpEnd - tmpStart + 1);
	while(current->score != INT64_MAX && numPre-- && limitTmp--)
	{
		NodeValue valueTmp;
		valueTmp.member = current->member;
		if(withscores)
		{
			valueTmp.score = current->score;
		}
		resList.push_back(valueTmp);
		if(reverse)
		{
...
void SkipList::getRangeByScore(const int64_t& start,const int64_t& end, std::vector<NodeValue>& resList, const bool& withscores,const int64_t& limitCount)
{
	if(start > end)
	{
		return ;
	...
	{
		NodeValue valueTmp;
		valueTmp.member = leftFirst->member;
		if(withscores)
		{
			valueTmp.score = leftFirst->score;
		}
		resList.push_back(valueTmp);
...
void SkipList::skdeletenode(SKNode *x,SKNode **update)
{
	...
	while(listLevel>1 && header->level[listLevel-1].forward->level[listLevel-1].forward->score==INT64_MAX)
	{
#ifdef CODE_TEST_LOG
		printf("listLevel--\n");
#endif
...
void SkipList::destroySkipList()
{
	int64_t tmpStart = 0;
	int64_t tmpEnd = length - 3;
	if(tmpStart>tmpEnd)
	{
		return;
	}
	...
	int64_t index = 0;
	int64_t numPre = (tmpEnd - tmpStart);
	while(current && current->score != INT64_MAX && (index <= numPre))
	{
		current = current->level[0].forward;
		skdeletenode(tmp, update);
		delete tmp;
